...
# ---------------------------------------------------------------
# TYPES_VETEMENTS CORRIGE - ampleurs et evasements ajustes
# ---------------------------------------------------------------
TYPES_VETEMENTS = {
    "Mini-jupe droite": {
        "categorie": "jupe",
    ...
    "Mini-jupe ovale": {
        ...
        "longueur_relative": 0.15,
        "ampleur": 1.5,
        "description": "Mini-jupe ovale volume rond prononce (mi-cuisse)"
    },
    "Jupe ovale au genou": {
        "categorie": "jupe",
        "type": "ovale",
        "longueur_relative": 0.35,
        "ampleur": 1.6,
        "description": "Jupe ovale volume rond tres prononce (genou)"
    },
    "Jupe trapeze au genou": {
        "categorie": "jupe",
        "type": "trapeze",
        "longueur_relative": 0.35,
        "evasement": 1.5,
        "description": "Jupe trapeze A-line depuis la taille (genou)"
    },
}
...
class VetementGenerator:
    ...
    def creer_mesh_jupe_separe(verts_corps, masque_jupe, couleur_nom):
        ...
        try:
            ...
            print(f"DENSITE MAXIMALE: {n_couches} couches x {n_points_par_cercle} points/cercle")
            rayons_par_hauteur = []
            ...
            if len(faces) > 0:
                try:
                    ...
                    print(f"MESH PARFAIT ZERO TROU: {len(points_jupe)} pts, {len(faces)} faces")
                    return {
    ...
    # ---------------------------------------------------------------
    # CORRECTION 1 : calculer_profil_jupe_droite
    # AVANT : rayons trop grands (x1.08, x1.12, x1.25) = jupe ample
    # APRES : rayons serrés (x1.02, x1.05, x1.08) = épouse les courbes
    # ---------------------------------------------------------------
    @staticmethod
    def calculer_profil_jupe_droite(points_anat, longueur_relative):
        """
        JUPE DROITE = Epouse les courbes du corps, tombe verticalement
        Suit la taille, les hanches et les cuisses sans evasement.
        """
        ...
        # Rayons serrés : epouse le corps sans evasement
        rayon_debut = rayon_taille * 1.02
        rayon_hanches_jupe = rayon_hanches * 1.05
        rayon_bas = rayon_hanches * 1.08
        ...
        print(f"PROFIL JUPE DROITE (epouse les courbes):")
        print(f"   Taille  : {rayon_debut:.3f}m (corps={rayon_taille:.3f}m x 1.02)")
        print(f"   Hanches : {rayon_hanches_jupe:.3f}m (corps={rayon_hanches:.3f}m x 1.05)")
        print(f"   Bas     : {rayon_bas:.3f}m (hanches x 1.08)")
    ...
    # ---------------------------------------------------------------
    # CORRECTION 2 : calculer_profil_jupe_ovale
    # AVANT : rayon_max = hanches * ampleur (trop plat, ressemblait a droite)
    # APRES : rayon_max = hanches * ampleur * 1.4 + gonflement au milieu
    # ---------------------------------------------------------------
    @staticmethod
    def calculer_profil_jupe_ovale(points_anat, longueur_relative, ampleur=1.4):
        """
        JUPE OVALE = Volume rond tres prononce, plis a la taille
        Forme en cloche : serree a la taille, tres gonflee au milieu,
        se referme legerement vers le bas.
        """
        y_taille = points_anat['y_taille']
        y_hanches = points_anat['y_hanches']
        y_min = points_anat['y_min']
        ...
        # Taille serree pour accentuer le contraste avec le volume
        rayon_debut = rayon_taille * 0.88

        # Volume MAXIMUM tres prononce (ampleur elevee = effet ballon/cloche)
        rayon_max = rayon_hanches * ampleur * 1.4

        # Bas se referme legerement (effet cloche fermee)
        rayon_bas = rayon_hanches * ampleur * 0.9

        # Le gonflement max est au 40% depuis le haut de la jupe
        y_max_largeur = y_debut_jupe - (y_debut_jupe - y_bas_jupe) * 0.4

        print(f"PROFIL JUPE OVALE (volume rond prononce):")
        print(f"   Taille (debut)  : {rayon_debut:.3f}m (serree x 0.88)")
        print(f"   Max gonflement  : {rayon_max:.3f}m a y={y_max_largeur:.3f}")
        print(f"   Bas (referme)   : {rayon_bas:.3f}m")
    ...
    # ---------------------------------------------------------------
    # CORRECTION 3 : calculer_profil_jupe_trapeze
    # AVANT : rayon_bas = hanches * evasement = cloche gonflee
    # APRES : rayon_bas = taille * evasement = A-line lineaire depuis taille
    # ---------------------------------------------------------------
    @staticmethod
    def calculer_profil_jupe_trapeze(points_anat, longueur_relative, evasement=1.5):
        """
        JUPE TRAPEZE = A-line classique, evasee progressivement depuis la taille
        Ligne droite et reguliere de la taille jusqu'au bas, aucun gonflement.
        """
        y_taille = points_anat['y_taille']
        y_hanches = points_anat['y_hanches']
        y_min = points_anat['y_min']
        ...
        # Debut a la taille : ajustement minimal
        rayon_debut = rayon_taille * 1.02

        # Evasement BASE SUR LA TAILLE (pas les hanches)
        # C'est ce qui donne le vrai effet A-line depuis la taille
        rayon_bas = rayon_taille * evasement

        print(f"PROFIL JUPE TRAPEZE (A-line depuis la taille):")
        print(f"   Taille (debut) : {rayon_debut:.3f}m")
        print(f"   Bas (max evas) : {rayon_bas:.3f}m (taille x {evasement})")
        print(f"   Evasement lineaire progressif depuis la taille")
    ...
    # ---------------------------------------------------------------
    # CORRECTION 4 : _calculer_rayon_droite
    # AVANT : rayon_bas = hanches * 1.25 = trop ample
    # APRES : rayon_bas = hanches * 1.08 = suit le corps
    # ---------------------------------------------------------------
    @staticmethod
    def _calculer_rayon_droite(profil, y):
        """
        Jupe droite : suit les courbes naturelles du corps sans evasement notable.
        Transitions douces entre taille, hanches et cuisses.
        """
    ...
    # ---------------------------------------------------------------
    # CORRECTION 5 : _calculer_rayon_ovale
    # AVANT : courbe cosinus symétrique trop plate
    # APRES : acceleration quadratique pour gonfler vite = vraie cloche
    # ---------------------------------------------------------------
    @staticmethod
    def _calculer_rayon_ovale(profil, y):
        """
        Jupe ovale : gonflement rapide depuis la taille vers le max,
        puis descente douce qui se referme legerement vers le bas.
        """
        y_debut = profil['y_debut']
        y_bas = profil['y_bas']
        ...
        if y >= y_max_largeur:
            # Zone haute : taille -> point de max gonflement
            # Acceleration quadratique pour gonfler vite depuis la taille
            if y_debut == y_max_largeur:
                return rayon_debut
            t = (y_debut - y) / (y_debut - y_max_largeur)
            t_curve = t * t  # quadratique : gonfle vite
            return rayon_debut + t_curve * (rayon_max - rayon_debut)
        else:
            # Zone basse : max gonflement -> bas (se referme doucement)
            if y_max_largeur == y_bas:
                return rayon_max
            t = (y_max_largeur - y) / (y_max_largeur - y_bas)
            t_curve = 0.5 * (1 - np.cos(np.pi * t))
            return rayon_max - t_curve * (rayon_max - rayon_bas)

    # ---------------------------------------------------------------
    # CORRECTION 6 : _calculer_rayon_trapeze
    # AVANT : lineaire mais partait des hanches (cloche)
    # APRES : lineaire pur depuis la taille = vraie A-line
    # ---------------------------------------------------------------
    @staticmethod
    def _calculer_rayon_trapeze(profil, y):
        """
        A-line pure : evasement strictement lineaire de la taille vers le bas.
        Aucune courbe, aucun gonflement. t=0 en haut (taille), t=1 en bas.
        """
        y_debut = profil['y_debut']
        y_bas = profil['y_bas']
        rayon_debut = profil['rayon_debut']
        ...
        # Interpolation lineaire pure : progression constante et reguliere
            ...
            if len(idx_couche) < 8:
                ...
                if rayon_cible > 0:
                    ...
                    for angle in angles:
                        x_syn = rayon_cible * np.cos(angle)
                        z_syn = rayon_cible * np.sin(angle)
                        vertices_synthetiques.append([x_syn, y_target, z_syn])
        if len(vertices_synthetiques) > 0:
            vertices_synthetiques = np.array(vertices_synthetiques)
            verts_modifies = np.vstack([verts_modifies, vertices_synthetiques])
...
class Visualisateur3D:
    ...
    def configurer_camera_pieds_visibles(self, plotter, vertices):
        ...
        print(f"CAMERA CORRIGEE - PIEDS VISIBLES:")
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if elapsed > timeout_duration:
                    print(f"{func.__name__} a pris {elapsed:.1f}s (>{timeout_duration}s)")
                return result
            except Exception as e:
                elapsed = time.time() - start_time
...
print(f"Environnement: {'PRODUCTION' if IS_PRODUCTION else 'DEVELOPPEMENT'}")
...
<body>
    <div class="test-box">
        ...
        <p>Environnement: <strong>{'PRODUCTION' if IS_PRODUCTION else 'DEVELOPPEMENT'}</strong></p>
    ...
    <script>
        ...
        function updateTimer() {{
            document.getElementById('timer').textContent = `Temps ecoulé: ${{seconds}} secondes`;
            seconds++;
        }}
    ...
    <script>
        ...
        try {{
            ...
            function init() {{
                ...
                scene.add(dirLight);
                let isMouseDown = false, previousMouse = null;
                ...
                renderer.domElement.addEventListener('mousemove', (e) => {{
                    if (isMouseDown && previousMouse) {{
                        mesh.rotation.y += (e.clientX - previousMouse.x) * 0.01;
                        mesh.rotation.x += (e.clientY - previousMouse.y) * 0.01;
...
<head>
    ...
    <style>
        ...
        .color-indicator {{ display: inline-block; width: 20px; height: 20px; border-radius: 50%; background-color: rgb({couleur_rgb[0]}, {couleur_rgb[1]}, {couleur_rgb[2]}); border: 2px solid white; margin-right: 8px; vertical-align: middle; }}
...
<body>
    <div id="container">
        ...
        <div id="info"><div><strong>{type_vetement}</strong></div><div><span class="color-indicator"></span>{couleur}</div></div>
    </div>
    <script>
        ...
        let scene, camera, renderer, meshCorps, meshVetement;
        function init() {{
            try {{
                scene = new THREE.Scene();
                scene.background = new THREE.Color(0xf0f0f0);
                ...
                for (let i = 0; i < masqueVetement.length; i++) {{
                    if (masqueVetement[i]) {{ verticesVetOnly.push(...verticesVetement[i]); indexMap.set(i, newIndex); newIndex++; }}
                }}
                for (let i = 0; i < faces.length; i++) {{
                    const face = faces[i];
                    if (masqueVetement[face[0]] && masqueVetement[face[1]] && masqueVetement[face[2]]) {{
                        const nv1 = indexMap.get(face[0]), nv2 = indexMap.get(face[1]), nv3 = indexMap.get(face[2]);
                        if (nv1 !== undefined && nv2 !== undefined && nv3 !== undefined) {{ facesVetOnly.push([nv1, nv2, nv3]); }}
                    }}
                }}
                if (verticesVetOnly.length > 0 && facesVetOnly.length > 0) {{
                    ...
                    meshVetement = new THREE.Mesh(geometryVet, new THREE.MeshLambertMaterial({{ color: new THREE.Color(couleurVetement[0], couleurVetement[1], couleurVetement[2]), transparent: true, opacity: 0.95 }}));
                ...
                renderer.domElement.addEventListener('mousemove', (e) => {{
                    if (isMouseDown && previousMouse) {{
                        const dx = (e.clientX - previousMouse.x) * 0.01;
                        const dy = (e.clientY - previousMouse.y) * 0.01;
                        if (meshCorps) {{ meshCorps.rotation.y += dx; meshCorps.rotation.x += dy; }}
                        if (meshVetement) {{ meshVetement.rotation.y += dx; meshVetement.rotation.x += dy; }}
                        previousMouse = {{ x: e.clientX, y: e.clientY }};
                    }}
                }});
                renderer.domElement.addEventListener('mouseup', () => {{ isMouseDown = false; }});
...
def health_check():
    return jsonify({
        'status': 'OK',
        'message': 'API Mannequin - JUPES CORRIGEES',
        ...
        'corrections_jupes': {
            'jupe_droite': 'Epouse les courbes (rayons x1.02, x1.05, x1.08)',
            'jupe_ovale': 'Volume rond prononce - cloche (rayon_max = hanches * ampleur * 1.4)',
            'jupe_trapeze': 'A-line depuis la taille (rayon_bas = taille * evasement)'
        },
        'timeouts': {
...
def generate_vetement():
    try:
        ...
        if params_vetement['type'] == 'droite':
            ...
            evasement = params_vetement.get('evasement', 1.5)
        ...
        try:
            ...
            if isinstance(mesh_vetement_data, dict):
                if mesh_vetement_data.get('mesh_object') is not None:
                    print(f"Mesh vêtement créé: {mesh_vetement_data.get('points_count', 0)} points")
                else:
                    print(f"Echec création mesh vêtement")
            else:
                mesh_vetement_data = {
        ...
        try:
            ...
            print(f"Vêtement sauvegardé: {temp_file}")
        except Exception as e:
        ...
        print(f"Vêtement généré: {vetement_id}")
        return jsonify({
            'success': True,
            'message': f'Vêtement {type_vetement} {couleur} généré',
            'info': info_convertie,
            ...
            'qualite': {
                ...
                'corrections_jupes': 'DROITE=epouse courbes, OVALE=cloche prononcee, TRAPEZE=A-line taille'
            }
        })
    except Exception as e:
...
<body>
    ...
    <div id="info">
        <div class="info-title"><span class="color-swatch"></span><span>{type_vetement}</span></div>
    </div>
    <script>
        let scene, camera, renderer, meshCorps, meshVetement;
        ...
        function init() {{
            ...
            for (let i = 0; i < masque.length; i++) {{
                if (masque[i]) {{ verticesVetOnly.push(...verticesVetement[i]); indexMap.set(i, newIdx); newIdx++; }}
            }}
            for (let i = 0; i < faces.length; i++) {{
                const [v1, v2, v3] = faces[i];
                if (masque[v1] && masque[v2] && masque[v3]) {{
                    const nv1 = indexMap.get(v1), nv2 = indexMap.get(v2), nv3 = indexMap.get(v3);
                    if (nv1 !== undefined && nv2 !== undefined && nv3 !== undefined) {{ facesVetOnly.push([nv1, nv2, nv3]); }}
                }}
            }}
            if (verticesVetOnly.length > 0 && facesVetOnly.length > 0) {{
            ...
            renderer.domElement.addEventListener('mousemove', (e) => {{
                if (isMouseDown && previousMouse) {{
                    rotationY += (e.clientX - previousMouse.x) * 0.008;
                    rotationX += (e.clientY - previousMouse.y) * 0.008;
            ...
            let touchStart = null;
            renderer.domElement.addEventListener('touchstart', (e) => {{ e.preventDefault(); if (e.touches.length === 1) {{ touchStart = {{ x: e.touches[0].clientX, y: e.touches[0].clientY }}; }} }});
            renderer.domElement.addEventListener('touchmove', (e) => {{
                e.preventDefault();
                if (touchStart && e.touches.length === 1) {{
                    rotationY += (e.touches[0].clientX - touchStart.x) * 0.008;
                    rotationX += (e.touches[0].clientY - touchStart.y) * 0.008;
                    rotationX = Math.max(-Math.PI / 2, Math.min(Math.PI / 2, rotationX));
                    if (meshCorps) {{ meshCorps.rotation.order = 'YXZ'; meshCorps.rotation.y = rotationY; meshCorps.rotation.x = rotationX; }}
                    if (meshVetement) {{ meshVetement.rotation.order = 'YXZ'; meshVetement.rotation.y = rotationY; meshVetement.rotation.x = rotationX; }}
...
def generate_fallback_preview(vetement_id, type_vetement, couleur, vertices):
    try:
        ...
        return jsonify({'error': 'Impossible de générer le preview', 'details': str(e), 'vetement_id': vetement_id}), 500
...
def generate_mannequin():
    try:
        ...
        if gender not in genders_valides:
            return jsonify({'error': f'Genre non valide: {gender}', 'genders_disponibles': genders_valides}), 400
        mesures_default = {
            'tour_taille': 68, 'tour_hanches': 92,
            'tour_poitrine': 88, 'hauteur': 170, 'longueur_bras': 60
        }
    ...
    <script>
        ...
        function init() {{
            ...
            document.addEventListener('mousemove', (e) => {{
                ...
                rotationX += (e.clientY - prevY) * 0.005;
            ...
            document.addEventListener('touchmove', (e) => {{
                ...
                rotationY += (e.touches[0].clientX - touchStart.x) * 0.005;
                rotationX += (e.touches[0].clientY - touchStart.y) * 0.005;
                mannequin.rotation.order = 'YXZ';
                mannequin.rotation.y = rotationY;
                mannequin.rotation.x = rotationX;
                touchStart = {{ x: e.touches[0].clientX, y: e.touches[0].clientY }};
...
def visualize_mannequin_3d(mannequin_id):
    try:
        ...
        faces = data['faces']
        gender = str(data['gender'])
        print(f"Visualisation 3D mannequin: {mannequin_id}")
...
if __name__ == '__main__':
    ...
    print(f"Environnement: {'PRODUCTION (Render)' if IS_PRODUCTION else 'DEVELOPPEMENT (Local)'}")
    print(f"Base URL: {BASE_URL}")
    print(f"Port: {port}")
    app.run(
        host='0.0.0.0',
